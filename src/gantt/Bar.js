  ...
  return (
    <g class="bar">
      ...
      {data.map((v, i) => {
        ...
        return (
          <g title={title} key={i} style={{ cursor: 'pointer' }} onClick={handler}>
            {/* <text x={x1 - 4} y={TY} style={styles.text1}>{formatDay(new Date(reality_from))}</text>
            { v.reality_to ? <text x={x1 + w1 + 4} y={TY} style={styles.text2}>{formatDay(new Date(v.reality_to))}</text> : null} */}
